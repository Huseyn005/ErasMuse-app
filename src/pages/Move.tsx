...
const Move = () => {
    ...
    const findRoute = () => {
        const found = routes.find(r => r.from === from && r.to === to) || routes.find(r => r.from.includes(from.split(' ')[0]) && r.to.includes(to.split(' ')[0])) || routes[0];
        setRoute(found);
    };
                        ...
                        <div key={r.id} className="surface p-4 hover:shadow-md transition-shadow">
                            <div className="flex items-center justify-between gap-2">
                                <div className="font-semibold text-sm">
                                    {r.from} <ArrowRight className="inline w-3.5 h-3.5 mx-0.5 text-muted-foreground" /> {r.to}
                                </div>
