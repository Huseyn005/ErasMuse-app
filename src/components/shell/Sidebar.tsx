...
export function Sidebar() {
    ...
    return (
        <TooltipProvider delayDuration={0}>
            <aside className={cn('hidden lg:flex flex-col shrink-0 border-r border-border bg-sidebar h-screen sticky top-0', isCollapsed ? 'w-16' : 'w-64')}>
                {/* Navigation */}
                    ...
                    <Tooltip>
                        <TooltipTrigger asChild>
                            <button
                                ...
                                className={cn('w-full flex items-center gap-3 h-12 rounded-xl transition-colors text-muted-foreground hover:text-primary hover:bg-sidebar-accent', isCollapsed ? 'justify-center' : 'px-3')}
                                aria-label={isCollapsed ? t('sidebar.expand') : t('sidebar.collapse')}
                            >
                                {isCollapsed ? <PanelLeft className="w-6 h-6" /> : <PanelLeftClose className="w-6 h-6" />}
                                {!isCollapsed && <span className="text-sm font-semibold transition-opacity duration-300">{t('sidebar.collapse')}</span>}
                            </button>
                        </TooltipTrigger>
